...
def move_snake():
    global ate_food, snake_coords, direction
    head = snake_coords[-1]
    new_head = None
    ...
    if (new_head[0] < -300 or new_head[0] > 300 or
        new_head[1] < -300 or new_head[1] > 300):
        game_over()
        return
...
def update():
    # Disable automatic screen updates and manually update at the end
    screen.tracer(0)
